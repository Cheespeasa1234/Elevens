 ...
 * 
 * <pre>
 * - Added forEach(Consumer<T> cons);
 * - Added forEach(Consumer<T> cons, int start, int end);
 * - Added swap(int a, int b);
 * - Added buffer(Array a, Array b, Object buffer);
 * - Added dist(Array<Number> a, Array<Number> b);
 * - Added has(T obj);
 * - Added number type validation
 * - Added support for some edge cases in some methods
 * - Improved efficiency of reverse();
 * 
 * @author nlevison25
 * @version 1.2 - Sep 28 2022
 *
...
public class Array<T> implements Comparable<T> {
	...
	public Array<T> clone() {
		Array<T> a = new Array<T>(this.array);
		return a;
	}
}
